...
const StatCard = ({ icon: Icon, label, value, sub, color = 'var(--primary)', trend }) => (
  <div className="card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      ...
      {trend !== undefined && (
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: trend >= 0 ? 'var(--success)' : 'var(--danger)', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
    ...
    <div>
      <div style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-main)', lineHeight: 1.1 }}>{value}</div>
      <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.5rem' }}>{label}</div>
...
const FunnelBar = ({ label, count, total, color, icon: Icon }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
      <div style={{ width: '130px', fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', flexShrink: 0 }}>
        {Icon && <Icon size={14} color={color} />} {label}
      </div>
      <div style={{ flex: 1, background: 'var(--bg-app)', border: '1px solid var(--border)', borderRadius: '99px', height: '10px', overflow: 'hidden' }}>
        <div style={{
      ...
      <div style={{ width: '60px', textAlign: 'right', fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-main)' }}>{count} <span style={{ color: 'var(--text-muted)', fontWeight: 500 }}>({pct}%)</span></div>
    </div>
  );
};
...
const WeeklyBar = ({ day, height, count }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem', flex: 1 }}>
    <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)' }}>{count}</div>
    ...
    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 600 }}>{day}</div>
  </div>
);
...
export default function Analytics() {
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '2rem' }} className="fade-up">
      
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-main)' }}>
            <BarChart2 size={24} color="var(--primary)" />
            Analytics
          </h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            AI-powered insights on your job search performance.
            {isMock && <span style={{ marginLeft: '0.5rem', padding: '0.2rem 0.5rem', background: 'var(--warning)', color: 'var(--text-inverse)', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Sample Data</span>}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1.5rem' }}>
        ...
        <div className="card" style={{ padding: '1.5rem' }}>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.25rem', color: 'var(--text-main)' }}>Application Funnel</h3>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '1.5rem' }}>Conversion rate at each stage</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <FunnelBar label="Evaluated" count={displayTotal} total={displayTotal} color="var(--primary)" icon={Briefcase} />
            <FunnelBar label="Applied" count={displayApplied} total={displayTotal} color="var(--success)" icon={CheckCircle2} />
            <FunnelBar label="Redirected" count={displayRedirected} total={displayTotal} color="var(--info)" icon={Clock} />
            <FunnelBar label="Skipped" count={displaySkipped} total={displayTotal} color="var(--danger)" icon={XCircle} />
          </div>
        </div>

        {/* Weekly Activity */}
        <div className="card" style={{ padding: '1.5rem' }}>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.25rem', color: 'var(--text-main)' }}>Weekly Activity</h3>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '1.5rem' }}>Applications submitted per day</p>
      ...
      <div className="card" style={{ padding: '1.5rem' }}>
        <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.25rem', color: 'var(--text-main)' }}>Portal Performance</h3>
        <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '1.5rem' }}>Breakdown of applications by job portal</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem' }}>
          {(Object.keys(portalCounts).length > 0 ? Object.entries(portalCounts) : [
            ...
            const colors = { linkedin: '#3b82f6', naukri: '#60a5fa', internshala: '#38bdf8', instahyre: '#10b981', unstop: '#f97316', foundit: '#8b5cf6', indeed: '#2563eb' };
            const color = colors[portal.toLowerCase()] || 'var(--primary)';
            return (
              <div key={portal} style={{ padding: '1.25rem 1rem', background: `${color}0F`, border: `1px solid ${color}25`, borderRadius: '12px', textAlign: 'center' }}>
                <div style={{ fontSize: '1.75rem', fontWeight: 800, color }}>{count}</div>
                <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-main)', marginTop: '0.25rem' }}>{portal}</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.1rem' }}>{Math.round((count / displayTotal) * 100)}% of total</div>
              </div>
            );
          })}
        </div>
      ...
      {/* AI Insights */}
      <div className="glass-panel" style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--accent)', fontWeight: 700, fontSize: '0.95rem' }}>
          <Zap size={16} /> AI Insights & Recommendations
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', width: '100%' }}>
          {[
            ...
            <div key={i} style={{ padding: '1rem', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
              <div style={{ padding: '0.4rem', background: `${color}15`, borderRadius: '8px', flexShrink: 0, display: 'flex' }}>
                <Icon size={14} color={color} />
              </div>
              <p style={{ fontSize: '0.78rem', lineHeight: 1.5, color: 'var(--text-muted)', fontWeight: 500 }}>{text}</p>
            </div>
          ))}
        </div>
      </div>
